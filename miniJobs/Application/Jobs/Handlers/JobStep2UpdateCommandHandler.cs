...
sealed class JobStep2UpdateCommandHandler(IJobRepository jobRepository, IJobQuestionRepository jobQuestionRepository,
IJobQuestionAnswerRepository jobQuestionAnswerRepository) : IRequestHandler<JobStep2UpdateCommand, Job>
{
    public async Task<Job> Handle(JobStep2UpdateCommand command, CancellationToken cancellationToken)

    {
        using var ts = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled);
        Job job = await jobRepository.TryFindAsync(command.Request.Id);

        job.JobTypeId = command.Request.JobTypeId;
        job.RequiredEmployees = command.Request.RequiredEmployees;
        job.ApplicationsDuration = command.Request.ApplicationsDuration;
