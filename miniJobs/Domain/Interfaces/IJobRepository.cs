...
public interface IJobRepository : IGenericRepository<Job, int>
{
    ...
    Task<IEnumerable<JobCardDTO>> SearchAsync(string searchText, int limit, int offset, int? cityId, SortOrder sort);
    Task<int> SearchCountAsync(string searchText, int? cityId);
    Task<IEnumerable<JobCardDTO>> GetApplicantSavedJobsAsync(int applicantId);
